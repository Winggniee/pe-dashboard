...
⚠️ **注意这跟 PE 统计看板（柱状图）是两套完全独立的过滤逻辑**：柱状图那边的排除条件在
`server/index.js` / `api/index.js` 的 `INACTIVE_STATUSES` 常量里（见 `CHANGELOG.md`
2026-07-22 的记录），数据源、字段结构、代码路径都不一样，两处互不调用。业务概念上两边
应该保持同步（同一个状态如果在甘特图里算"结束"，柱状图里通常也该算"不活跃"），但**改
一边不会自动影响另一边**，需要分别修改。目前两边覆盖的状态集合略有差异：
- 甘特图 `CLOSED_STATUSES`：`已流失`、`封号`、`项目暂停`、`已移交`
- 柱状图 `INACTIVE_STATUSES`：`封号`、`客户封号`、`已流失`、`项目暂停`、`已移交`
  （多了 `客户封号`，因为柱状图的数据源表里这个状态用了不同的确切字符串）
