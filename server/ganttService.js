...
//
// 已流失/封号/项目暂停/已移交 are never rendered as their own colored/labeled
// segment — reaching any of these statuses ends the project. The previous
// segment (if any) is closed at that event's timestamp; if there is no previous
// segment (the very first known event for a run is already one of these), a
// neutral "ended" segment is emitted instead of fabricating a status.
const feishuApi = require('./feishuApi');

const CLOSED_STATUSES = ['已流失', '封号', '项目暂停', '已移交'];
